...
layout (binding = 0) uniform sampler2D textureSample1;
layout (binding = 1) uniform sampler2D textureSample2;
...
void main()
{
	...
	vec4 textureColor2 = texture(textureSample2, fragment_uv);
	...
	color =  (textureColor1 * vec4(ambient + diffuse, 1.0)) + (textureColor2 * vec4(specular, 1.0));
}